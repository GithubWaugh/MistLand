...
Layers (bottom to top) :
  ...
  2. Data overlay : water / temperature / pressure — exclusive, toggle 1/2/3
  3. Veg icons    : drawn per cell — always visible, toggle 4

Controls :
  SPACE       : step one tick (when paused)
  A           : pause / resume
  PageUp      : speed up   (double tick rate)
  PageDown    : slow down  (halve tick rate)
...
# Tick speed steps (ticks per second)
SPEED_STEPS     = [0.25, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0]
SPEED_DEFAULT   = 2   # index into SPEED_STEPS → 1.0 tick/s
...
def run(world: World) -> None:
    ...
    tick_accum     = 0.0   # accumulated time for sub-second speeds
    ...
    def render():
        ...
        ov_names  = {OV_NONE: "---", OV_WATER: "water",
                     OV_TEMP: "temp", OV_PRESSURE: "pressure"}
        speed_tps = SPEED_STEPS[speed_idx]
        speed_str = (f"{speed_tps:.2f}".rstrip('0').rstrip('.') + " t/s")
        ...
        info_str = (
            ...
            f"[1]water [2]temp [3]press [4]veg  [ESC] quit"
        )
